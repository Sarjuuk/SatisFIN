ExtFiles  = {
    ...
    '/include/logger.lua',
    '/include/shared-prod.lua'
}

PlantName = 'plasticPlant'
...
Net[Net.msg.REQ_PLANT_STATUS[1]] = function(self, srcUUID, name, qty)
    ...
    for n, data in pairs(Plant.stats.prod) do
        Net:send(srcUUID, Net.ports.master, 'RSP_PR_STATE', n, 'plastic', data.cur, data.max)  -- what, mat, cur, max
    end

    for n, data in pairs(Plant.stats.buffer) do
        Net:send(srcUUID, Net.ports.master, 'RSP_BU_STATE', n, 'plastic', data.cur, data.max)  -- what, mat, cur, max
    end
end
...
Net:init(PlantName, Control:getPanelElement('NetIn', nil, nil, 1), Control:getPanelElement('NetOut', nil, nil, 1))
